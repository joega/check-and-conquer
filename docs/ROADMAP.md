...
**Progress:** Implemented for the current Linux target. The playable screen keeps a board-first view with one Menu button for optional manual UCI entry, restart, undo, player-versus-Stockfish and spectator controls, side and Novice/Master selectors, promotion piece, capture speed, fullscreen, master volume, camera shake, and bounded Stockfish diagnostics. It opens directly with keyboard/controller focus on Play vs Stockfish. A balanced direct player-side turn view frames every board corner with a readable foreground team and remains behind the human player while Stockfish moves from the far side. Quiet moves deliberately retain that player-controlled board view, avoiding a disruptive camera transition while a player evaluates the position. A live corner readout exposes the editable camera tilt, spin, zoom, and pan values for view tuning. Local play is deliberately unavailable in V1. The board is a bronze-railed stone altar framed by an original high-detail mountain-summit panorama; a subtle cyan levitation ring, underglow, and drifting ritual glyphs turn the panoramic scene’s depth boundary into an intentional fantasy effect. Lightning and blue-flame beacons remain outside the gameplay sightline. The setting is isolated from chess state so it can be freely rebuilt. These player settings, including fullscreen, persist in `user://warchessed_settings.cfg`. The Linux export stages Stockfish externally, includes the required notices/source, and has been smoke-launched headlessly as `warchessed.x86_64`.
