...
**Progress:** Implemented for the current Linux target. The playable screen has a compact board-first toolbar with optional manual UCI entry in Settings, a computer/local toggle, side and Novice/Master selectors, selectable promotion piece, restart, domain-backed undo, a game-over panel, capture skip and speed controls, right-drag orbit/middle-drag pan/mouse-wheel zoom, a high direct player-side turn-view behind the active team plus a shoulder-offset walking follow shot, fullscreen toggle, master volume, optional camera shake, and a bounded Stockfish diagnostic log. The board sits on a procedural storm-mountain plateau with drifting clouds, rain, deterministic lightning, and blue-flame beacons, isolated from chess state so it can be freely rebuilt. These player settings, including fullscreen, persist in `user://warchessed_settings.cfg`. The Linux export stages Stockfish externally, includes the required notices/source, and has been smoke-launched headlessly as `warchessed.x86_64`.
