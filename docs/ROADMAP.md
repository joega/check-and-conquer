...
**Progress:** Implemented for the current Linux target. The playable screen has a compact board-first toolbar with optional manual UCI entry in Settings, player-versus-Stockfish and spectator modes, side and Novice/Master selectors, selectable promotion piece, restart, domain-backed undo, a game-over panel, capture skip and speed controls, right-drag orbit/middle-drag pan/mouse-wheel zoom, a high direct player-side turn-view behind the active team plus a shoulder-offset walking follow shot, fullscreen toggle, master volume, optional camera shake, and a bounded Stockfish diagnostic log. Local play is deliberately unavailable in V1. The board sits on a procedural storm-mountain plateau with rain, deterministic lightning, and blue-flame beacons outside the gameplay view, isolated from chess state so it can be freely rebuilt. These player settings, including fullscreen, persist in `user://warchessed_settings.cfg`. The Linux export stages Stockfish externally, includes the required notices/source, and has been smoke-launched headlessly as `warchessed.x86_64`.
