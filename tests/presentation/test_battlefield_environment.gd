...
func _run() -> void:
	...
	assert(is_equal_approx(battlefield._valley_vertex(36.0, -36.0).y, -0.32), "The player camera must remain inside the unobstructed valley floor.")
	assert(battlefield._valley_vertex(72.0, 72.0).y > 5.0, "Terrain outside the board valley must rise into surrounding mountains.")
