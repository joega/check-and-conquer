...
func _run() -> void:
	...
	assert(not screen.get_node("UI/CameraDebug").visible, "Normal play must hide camera telemetry.")
	...
	assert(screen.get_node("ChessBoard")._last_move_squares.size() == 2, "Last move must remain visible beyond the old timeout while deciding.")
	...
	assert(screen.get_node("ChessBoard")._last_move_squares.is_empty(), "Starting a new spectator match clears the previous move cue.")
	...
	assert(screen.get_node("ChessBoard")._last_move_squares.is_empty() and screen.get_node("ChessBoard")._hint_squares.is_empty(), "Review must not display cues from the live position.")
